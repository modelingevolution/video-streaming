...
namespace ModelingEvolution.VideoStreaming.CameraSimulator
{
    ...
    internal class Program
    {
        ...
        static void RunInTheFly(string[] args)
        {
            if (args.Length < 2)
            {
            ...
            using SharedCyclicBuffer buffer = new SharedCyclicBuffer(120, frameSize, streamName, OpenMode.OpenExistingForWriting);
            buffer.Clear();
            int c = 0;
            while (true)
            {
                ...
                if (!isSuccess)
                {
                    ...
                    Console.WriteLine("Reopening the file.");
                }
        ...
        private static void RunInMemory(string[] args)
        {
            if (args.Length < 2)
            {
                Console.WriteLine("Args: [mp4 file] [stream name]");
