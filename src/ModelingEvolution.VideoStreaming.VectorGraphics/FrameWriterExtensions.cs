...
public static class FrameWriterExtensions
{
    ...
    public static void WriteFrameNumber(this ArrayBufferWriter<byte> buffer, ulong frame)
    {
        MemoryMarshal.Write(buffer.GetSpan(sizeof(ulong)), frame);
        buffer.Advance(sizeof(long));
    }

    public static void WriteFrameEnd(this ArrayBufferWriter<byte> buffer)
    {
    ...
    public static void WriteFramePayload<T>(this ArrayBufferWriter<byte> buffer, ushort type, T value)
    {
