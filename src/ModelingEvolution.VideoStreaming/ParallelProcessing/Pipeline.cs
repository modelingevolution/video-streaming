...
namespace ModelingEvolution.VideoStreaming.ParallelProcessing
{
    public sealed class WorkPipeline<TIn, TOut>(int maxParallelItems,
        Func<CancellationToken, TIn> getWorkItem,
        Func<TIn, ulong, CancellationToken, TOut> process,
        Action<TOut, CancellationToken> mergeResults) : IDisposable
    {
        readonly record struct InData(ulong SeqNo, CancellationToken Token, TIn Data);
        ...
        public void Start(CancellationToken token = default)
        {
            ...
            _dispatcher.Name = "pipeline_dispatcher";
            _dispatcher.Start();

            _merger = new Thread(OnMergeResults);
            _merger.IsBackground = true;
            _merger.Name = "pipeline_merger";
            _merger.Start();
        }
        ...
        private void OnProcess(object state)
        {
            ...
            OnProcess(i.Data, i.Token, i.SeqNo);

        }
        private void OnProcess(TIn data, CancellationToken token, ulong seqNo)
        {
            ...
            try
            {
                var ret = process(data, seqNo, token);
        ...
        private void OnDispatch()
        {
            ...
            try
            {
                while (!_ct.IsCancellationRequested)
                {
                    ...
                    if (!ThreadPool.QueueUserWorkItem(OnProcess, new InData(i++, _ct, item)))
